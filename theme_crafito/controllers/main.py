...
class CrafitoEcommerceShop(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if session.get('sort_id'):
            ...
            sort = session_sort
            sort_field = request.env['biztech.product.sortby'].sudo().search([('id', '=', int(sort))])
...
# ==============================
    ...
    def wholesale_page(self,page=0, category=None, search='', ppg=False, **post):
        if ppg:
            try:
                ppg = int(ppg)
            except ValueError:
        ...
        keep = QueryURL('/wholesale', category=category and int(category), search=search, attrib=attrib_list, order=post.get('order'))
    ...
    @http.route('/wholesale', type='http', auth='public', website=True)
    def render_example_page(self,page=0, category=None, search='', ppg=False, **post):
